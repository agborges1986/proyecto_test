...
class Tool(models.Model):
    ...
    assigned_at=models.ForeignKey(Employee,related_name='assigned_at', on_delete=models.DO_NOTHING, null=True)
    belong_to=models.ForeignKey(Warehouse,related_name='belong_to', on_delete=models.DO_NOTHING, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    active = models.BooleanField(default=True)
...
class Certification(models.Model):
    tool=models.ForeignKey(Employee,related_name='certification', on_delete=models.DO_NOTHING, null=True)
    is_necessary = models.BooleanField(default=False)
    last_certification = models.DateTimeField()
    certification_period=models.IntegerField(default=180)  #180 dìas (6 meses) de certificación por defecto
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
